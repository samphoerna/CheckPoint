...
How to Run via Command (testing)
export PATH=$PATH:$(go env GOPATH)/bin && wails dev

How to Build via Command (onliner)
export PATH=$PATH:$(go env GOPATH)/bin && \
wails build -platform windows/amd64 -ldflags "-s -w -X main.AppVersion=V0.1.34c1" -clean && \
mv "build/bin/CheckPoint.exe" "build/bin/CheckPoint_V0.1.34c1.exe" && \
wails build -platform darwin/universal -ldflags "-s -w -X main.AppVersion=V0.1.34c1" && \
mv "build/bin/CheckPoint.app" "build/bin/CheckPoint_V0.1.34c1.app"
...
### macOS (.app)
1.  **Download** the `CheckPoint_v0.1.34.dmg` image (or `.zip`).
2.  **Open** the DMG file.
...
2.  **Naming Convention:** Backups are stored in a dedicated folder.
    ...
    - Example: `/backup/CheckPoint_V0.1.33a`
